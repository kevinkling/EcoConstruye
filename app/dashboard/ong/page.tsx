...
import type { Material, Request, User, MaterialCategory } from "@/lib/types"
...
const materialCategories: { value: MaterialCategory | "all"; label: string }[] = [
...
export default function ONGDashboard() {
  ...
  useEffect(() => {
    ...
    const availableMaterials = mockMaterials.filter((m) => m.status === "disponible")
    setMaterials(availableMaterials)
    setFilteredMaterials(availableMaterials)

    // Filtrar solicitudes de esta ONG
    setMyRequests(mockRequests.filter((r) => r.ong_id === currentUser.id))
  }, [])
  ...
  useEffect(() => {
    ...
    // Filtrar por ciudad
    if (selectedCity !== "all") {
      filtered = filtered.filter((material) => material.obra_location.city === selectedCity)
    }
  ...
  const cities = Array.from(new Set(materials.map((m) => m.obra_location.city)))
                          ...
                          <CardHeader>
                            <div className="flex justify-between items-start">
                              <div>
                                ...
                                <p className="text-gray-600 mt-1">{material.obra_location.city}</p>
                              </div>
                              ...
                              {request.status === "aprobada" && (
                                <div className="bg-green-50 p-4 rounded-lg border border-green-200">
                                  ...
                                  <p className="text-green-700 text-sm mb-3">
                                    Puedes coordinar la recogida del material contactando directamente con la obra.
                                  </p>
                                  <div className="space-y-1 text-sm">
                                    ...
                                    <p>
                                      <strong>Dirección:</strong> {material.obra_location.address},{" "}
                                      {material.obra_location.city}
                                    </p>
                                    <p>
                                      <strong>Horarios:</strong> {material.pickup_schedule.days.join(", ")} de{" "}
                                      {material.pickup_schedule.start_time} a {material.pickup_schedule.end_time}
                                    </p>
                                    {material.pickup_schedule.special_instructions && (
                                      <p>
                                        <strong>Instrucciones:</strong> {material.pickup_schedule.special_instructions}
                                      </p>
                                    )}
                                  </div>
                                </div>
