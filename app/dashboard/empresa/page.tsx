...
import type { Material, Request, User } from "@/lib/types"
...
export default function EmpresaDashboard() {
  ...
  useEffect(() => {
    ...
    setMaterials(mockMaterials.filter((m) => m.empresa_id === currentUser.id))
    // Filtrar solicitudes para materiales de esta empresa
    const empresaMaterialIds = mockMaterials.filter((m) => m.empresa_id === currentUser.id).map((m) => m.id)
    setRequests(mockRequests.filter((r) => empresaMaterialIds.includes(r.material_id)))
  }, [])
  ...
  const stats = {
    ...
    completedDonations: materials.filter((m) => m.status === "entregado").length,
  }
